create or replace function public.get_public_quiz_questions_by_law(
  p_law_slug text,
...
as $$
  ...
  where l.slug = p_law_slug
    and q.is_published
    and q.verified_at is not null
    and not q.needs_review
    and not q.is_prime
  group by q.id, q.text, q.difficulty, q.explanation, l.title, a.article_number, q.law_article, a.version_date, q.law_version_date
  having count(o.id) = 4 and count(*) filter (where o.is_correct) = 1
  order by random()
  limit least(greatest(p_count, 1), 200)
$$;

grant execute on function public.get_public_quiz_questions_by_law(text, int) to anon, authenticated;

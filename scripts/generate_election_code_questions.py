...
DOCX = ROOT / "ბაზა კანონების" / "საქართველოს საარჩევნო კოდექსი.docx"
CSV_OUT = ROOT / "data" / "election_code_questions.csv"
SQL_OUT = ROOT / "supabase" / "import_election_code_questions.sql"

LAW_NAME = "საქართველოს საარჩევნო კოდექსი"
LAW_GENITIVE = "საქართველოს საარჩევნო კოდექსის"
LAW_IN = "საქართველოს საარჩევნო კოდექსში"
LAW_TITLE = "საქართველოს ორგანული კანონი „საქართველოს საარჩევნო კოდექსი“"
LAW_SLUG = "election-code"
CATEGORY_SLUG = "constitutional"
SUBCATEGORY_SLUG = "election-code"
...
def sentence_candidates(article: dict[str, str]) -> list[str]:
    text = clean_space(article["content"].replace("\n", " "))
    ...
    for piece in pieces:
        ...
        if not (55 <= len(piece) <= 230):
            continue
        if any(bad in piece for bad in ["№", "ვებგვერდი", "ამოღებულია", "ძალადაკარგულია"]):
            continue
        if not any(key in piece for key in [
            "უფლება", "ვალდებულ", "ეკრძალ", "ტარდება", "ადგენ", "არის", "შეიძლება",
            "უნდა", "არ შეიძლება", "განისაზღვრ", "ნიშნავს", "მონაწილეობ", "კენჭისყრ",
            "არჩევნ", "ამომრჩევ", "კომისი", "რეფერენდუმ", "საჩივარ",
        ]):
            continue
        out.append(piece)
    return out[:3]
...
def build_rows(articles: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    for article in articles:
        ...
        if article_statements:
            ...
            text = f"{LAW_GENITIVE} მიხედვით, რომელი დებულება შეესაბამება საკითხს „{item['title']}“?"
...
begin
  ...
  values ('{CATEGORY_SLUG}', 'კონსტიტუციური სამართალი', 'კონსტიტუციური და საარჩევნო სამართლის ტესტები.', 5, true)
  ...
  values (v_category_id, 'საქართველოს საარჩევნო კოდექსი', '{SUBCATEGORY_SLUG}')
  ...
  values ('{LAW_SLUG}', {sql_quote(LAW_TITLE)}, 'საარჩევნო კოდექსი', '{CATEGORY_SLUG}', 'საქართველოს საარჩევნო კოდექსის მიხედვით შედგენილი სიტყვასიტყვითი და პრაქტიკული ტესტები.', '{VERSION_DATE}'::date, true, false, 70)
...
def validate(rows: list[dict[str, str]]) -> None:
    ...
    for index, row in enumerate(rows, start=1):
        ...
        assert "საქართველოს საარჩევნო კოდექს" in row["text"], index

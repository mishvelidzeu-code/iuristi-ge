...
if (!inputPath || !outputPath) {
  throw new Error('Usage: node scripts/build-penitentiary-law-import.mjs <input.json> <output.sql>');
}
...
if (!Array.isArray(questions) || questions.length !== 150) {
  throw new Error('Expected exactly 150 questions from the source document.');
}
...
-- The supplied document contains question text, four answer choices, and an answer key.
do $$
declare
  v_category_id uuid;
  ...
  r record;
begin
  select id into v_category_id from public.categories where slug = 'criminal';
  if v_category_id is null then
    raise exception 'Criminal category is missing';
  end if;

  insert into public.subcategories (category_id, slug, title)
  values (v_category_id, 'penitentiary-law', 'სასჯელაღსრულების სამართალი')
  ...
  insert into public.laws (
    ...
    'penitentiary-law', 'სასჯელაღსრულების სამართალი', 'სასჯელაღსრულების სამართალი', 'criminal',
    'სასჯელაღსრულების სამართლის ტესტები, დოკუმენტის კითხვებისა და პასუხების სიის მიხედვით.',
    '', current_date, true, false, 56, now()
  )
  on conflict (slug) do update set
    title = excluded.title, short_title = excluded.short_title,
    direction_slug = excluded.direction_slug, description = excluded.description,
    official_url = excluded.official_url, current_version_date = excluded.current_version_date,
    is_active = true, needs_review = false, sort_order = excluded.sort_order, updated_at = now()
  ...
  loop
    ...
    if v_question_id is null then
      insert into public.questions (
        ...
        'სასჯელაღსრულების სამართალი', r.question->>'law_article', '', current_date,

...
DOCX = ROOT / "ბაზა კანონების" / "საქართველოს სისხლის სამართლის კოდექსი.docx"
CSV_OUT = ROOT / "data" / "criminal_code_questions.csv"
SQL_OUT = ROOT / "supabase" / "import_criminal_code_questions.sql"

CATEGORY_SLUG = "criminal"
SUBCATEGORY_SLUG = "criminal-code"
LAW_SLUG = "criminal-code"
LAW_NAME = "საქართველოს სისხლის სამართლის კოდექსი"
LAW_GENITIVE = "საქართველოს სისხლის სამართლის კოდექსის"
LAW_SHORT_TITLE = "სისხლის სამართლის კოდექსი"
LAW_DESCRIPTION = "საქართველოს სისხლის სამართლის კოდექსის მიხედვით შედგენილი კაზუსური ტესტები."
OFFICIAL_URL = "https://matsne.gov.ge/ka/document/view/16426"
...
def build_question(article: Article, version_date: str) -> dict[str, str]:
    if is_removed(article):
        scenario = (
            ...
            "როგორც დღეს მოქმედი დამოუკიდებელი დანაშაულის შემადგენლობა."
        )
        prompt = "საქართველოს სისხლის სამართლის კოდექსის მოქმედი ტექსტის შესაბამისად, რომელი შეფასებაა სწორი?"
        correct = f"{article.law_article} მოქმედ რედაქციაში ამოღებულია და დამოუკიდებელ შემადგენლობად ვერ გამოიყენება."
        wrongs = [
            f"{article.law_article} მოქმედებს სრული მოცულობით და დამატებით შემოწმებას არ საჭიროებს.",
            "ამოღებული მუხლი შეიძლება გამოყენებულ იქნეს, თუ ბრალდებული თანახმაა ასეთ კვალიფიკაციაზე.",
            "ამოღებული მუხლი გამოიყენება მხოლოდ მაშინ, როცა საქმე ნაკლებად მძიმე დანაშაულს ეხება.",
        ]
        ...
        correct = (
            f"თუ დადასტურდება მუხლში მითითებული ყველა ნიშანი, ქმედება უნდა შეფასდეს "
            f"{article.law_article}-ით გათვალისწინებულ დანაშაულად."
        )
        wrongs = [
            "ქმედება სისხლისსამართლებრივად არ ფასდება და მხოლოდ ადმინისტრაციულ პასუხისმგებლობას იწვევს.",
            "ქმედება ამ მუხლით ფასდება მხოლოდ მაშინ, თუ იგი ჩადენილია არაერთგზის.",
            "ქმედება ამ მუხლით ფასდება მხოლოდ მაშინ, თუ ბრალდებული თანამდებობის პირია.",
        ]
        explanation_core = f"ადგენს შესაბამისი ქმედების დანაშაულებრივობას და სანქციას."
    else:
...
begin
  ...
  values ('{CATEGORY_SLUG}', 'სისხლის სამართალი', 'დანაშაული, სასჯელი და სისხლის სამართლის პროცესი.', 10, true)
